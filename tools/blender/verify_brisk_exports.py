"""Verify the separate Brisk driver, skinning, blink, and portable file round trip."""
import hashlib
import json
...
EXPORTS=OUT/'exports'
manifest=json.loads((EXPORTS/'brisk-driver.export.json').read_text())
...
data=(EXPORTS/'brisk-driver.glb').read_bytes()
...
bpy.ops.wm.open_mainfile(filepath=str(OUT/'brisk-driver.blend'))
scene=bpy.context.scene
scene.frame_set(1)
rig=bpy.data.objects['Brisk_Driver_Rig']
...
assert all(lid.data.shape_keys.key_blocks['Blink'].value==0 for lid in lids)
scene.frame_set(44)
assert all(lid.data.shape_keys.key_blocks['Blink'].value==1 for lid in lids)
bpy.context.view_layer.update()
dg=bpy.context.evaluated_depsgraph_get()
for side,lid in zip(('L','R','Center'),lids):
    ...
    marker=bpy.data.objects['Eye_Catchlight_'+side].evaluated_get(dg)
    ...
    for v in marker_mesh.vertices:
        ...
        assert hit is not None and hit.y < p.y-.00001, (side,'Reflection protrudes through the closed eyelid')
...
bpy.ops.import_scene.fbx(filepath=str(EXPORTS/'brisk-driver.fbx'))
scene=bpy.context.scene
scene.frame_set(1)
meshes=[o for o in scene.objects if o.type=='MESH']
...
assert sum(bool(o.data.shape_keys) for o in meshes)==3
...
report={'passed':True,'driverOnlyNativeFile':True,'seatAndWheelExcluded':True,
        ...
        'blinkTargets':3,'blinkAtFrame44Verified':True,'weightsNormalized':True,
        'allThreeClosedLidsCoverEyeReflections':True,
...
print('BRISK_EXPORTS_VERIFIED '+json.dumps(report),flush=True)
