...
[Grit's driver](../art-source/blender/characters/grit/README.md) uses `build_grit.py`, `prepare_grit.py`, and `verify_grit_exports.py` under `tools/blender`. The stable Blender file contains only the rigged character; fitting guides stay in the review scene. Its FBX and GLB are staged in the character's `exports` folder, with source hashes and round-trip validation, before Unity prefab and material integration.

Grit's [Dune Hopper](../art-source/blender/karts/dune-hopper/README.md) uses `build_dune_hopper.py`, `finish_dune_hopper.py`, and `verify_dune_hopper.py`. Its combined scene keeps the accepted driver, seat, wheels, steering wheel, and carried spare independent. Eight grouped vehicle meshes are exported with UVs; the detailed authoring parts remain intact. A car-only FBX is staged under `Art/Models/Karts`, with car-only and animated combined GLBs beside the Blender source. The manifest records placement, material values, sockets, source hashes, and tested visual controls. Runtime physics, materials, LODs, and a wrapper prefab still need integration.
